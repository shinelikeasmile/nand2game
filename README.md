kkkkkkkkkkkkkkkkkkkkkkkkkkk# Nand2Game
...
* stack machine is an abstraction consists of an architecture (a stack) and a set of operations which we can apply to this architecture.
...
* The VM architecture could be implemented with relative ease on any given hardware platform .
...
* The VM Architecture provides an intermediate layer that hides most of the details of the target hardware from the high level language.
* Our VM language recognises only one data type but the Java's bytecode commands are designed to manipulate integers,floating point,double precision etc and it has more expressive power but both are stack based.
* push/pop segment i 
